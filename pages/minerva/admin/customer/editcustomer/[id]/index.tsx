...
export const getStaticPaths = async () => {
  const res = await fetch("http://localhost:3001/user/getUsers");
  const repo = await res.json();


  const paths = await repo.map(({ userID }: any) => {
...
export const getStaticProps = async (context: GetStaticPropsContext) => {
  const res = await fetch(`http://localhost:3001/user/getUsersId/${context.params?.id}`)
...
const EditCustomerPage: FC<InputProp> = ({ res }: any) => {
...
<div className=" w-full mx-auto">
         ...
         <div className="mb-6">
             ...
             <input type="text" id="firstName" onChange={(e) => setFirstname(e.target.value)} className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="Input first name" value={firstName} onChange={handleFirstNameChange} required />
             </div>
         <div className="mb-6">
