...
interface ModalProps {
  isOpenBankTrans: boolean;
  onCloseBankTrans: () => void;
  children: ReactNode;
}

const Modal: FC<ModalProps> = ({ isOpenBankTrans, onCloseBankTrans, children }) => {
  return isOpenBankTrans? (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-opacity-25 backdrop-blur-sm">
      <div className="w-[600px] flex flex-col relative z-50 p-8 max-w-md rounded-md text-left">
        <button onClick={onCloseBankTrans} className="absolute top-0 right-0 text-black text-xl place-self-end">
          X
        </button>
